...
import { paginateUsers, __allUsers, PaginateUserOptions } from '../mocks/usuarios.mock';
import { PaginationType } from '../types/pagination.type';

@Injectable({ providedIn: 'root' })
export class UsuarioService {
  list(page: number = 1, pageSize: number = 10, opts: PaginateUserOptions = {}): Observable<ResponseType<PaginationType<UsuarioType>>> {
  ...
  create(payload: Partial<UsuarioType>): Observable<ResponseType<UsuarioType>> {
  ...
  update(id: string, payload: Partial<UsuarioType>): Observable<ResponseType<UsuarioType | null>> {
